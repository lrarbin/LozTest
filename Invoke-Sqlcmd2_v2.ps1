...
function Invoke-Sqlcmd2 
{
    ...
    Begin
    {
        ...
        Write-Verbose -Message "Running Invoke-Sqlcmd2 with ParameterSet '$($PSCmdlet.ParameterSetName)'.  Performing query '$Query'"
...
'@
        ...
        if($PSBoundParameters.ContainsKey('SQLConnection'))
        {
            if($SQLConnection.State -notlike 'Open')
            {
                Try
                {
                    Write-Verbose -Message "Opening connection from '$($SQLConnection.State)' state"
                    $SQLConnection.Open()
                }
                Catch
                {
            ...
            if($Database -and $SQLConnection.Database -notlike $Database)
            {
                Try
                {
                    Write-Verbose -Message "Changing SQLConnection database from '$($SQLConnection.Database)' to $Database"
    ...
    Process
    {
        ...
        {
            Write-Verbose -Message "Querying ServerInstance '$SQLInstance'"
